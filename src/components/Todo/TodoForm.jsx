...
const TodoForm = ({ todo, categories, tags, onSubmit, onCancel }) => {
  ...
  const [formErrors, setFormErrors] = useState({}); // Pour stocker les erreurs de validation par champ
  ...
  useEffect(() => {
    ...
    setFormErrors({}); // Réinitialiser les erreurs lors de l'initialisation/changement de todo
  }, [todo]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const dataToSubmit = {
        ...
        // Convertir la date en format ISO pour l'API si elle existe
        due_date: formData.due_date ? new Date(formData.due_date).toISOString() : null,
      };
      await onSubmit(dataToSubmit);
      onCancel(); // Ferme le formulaire après soumission réussie
    } catch (err) {
      console.error("Form submission error:", err);
      if (err.response && err.response.data && err.response.data.errors) {
        setFormErrors(err.response.data.errors); // Laravel renvoie les erreurs par champ
      } else if (err.response && err.response.data && err.response.data.message) {
        setFormErrors({ general: err.response.data.message }); // Erreur générale
      } else {
